    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div className=''>
          ...
          <select onChange={(event)=>setSubCategory(event.target.value)} className='w-full px-3 py-2'>
            <option value="Topweare">Topweare</option>
        ...
        <div>
          ...
          <input onChange={(event)=>setPrice(event.target.value)} value={price} className='w-full px-3 py-2 sm:w-[120px]' type="Number" placeholder='300'/>
        </div>
      </div>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          <div onClick={(event)=>setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S" ) : [...prev, "S"])}>
            <p className={`${sizes.includes("S") ? 'bg-pink-100': 'bg-slate-200'} px-3 py-1 cursor-pointer`}>S</p>
          </div>

          <div onClick={(event)=>setSizes(prev => prev.includes("M") ? prev.filter(item => item !== "M" ) : [...prev, "M"])}>
            <p className={`${sizes.includes("M") ? 'bg-pink-100': 'bg-slate-200'} px-3 py-1 cursor-pointer`}>M</p>
          </div>

          <div onClick={(event)=>setSizes(prev => prev.includes("L") ? prev.filter(item => item !== "L" ) : [...prev, "L"])}>
            <p className={`${sizes.includes("L") ? 'bg-pink-100': 'bg-slate-200'} px-3 py-1 cursor-pointer`}>L</p>
          </div>

          <div onClick={(event)=>setSizes(prev => prev.includes("XL") ? prev.filter(item => item !== "XL" ) : [...prev, "XL"])}>
            <p className={`${sizes.includes("XL") ? 'bg-pink-100': 'bg-slate-200'} px-3 py-1 cursor-pointer`}>XL</p>
          </div>

          <div onClick={(event)=>setSizes(prev => prev.includes("XXL") ? prev.filter(item => item !== "XXL" ) : [...prev, "XXL"])}>
